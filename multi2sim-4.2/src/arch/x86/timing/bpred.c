...
void showbits(unsigned int x) {
	char *value = xcalloc(x86_bpred_gshare_hist, sizeof(char));
	int j = 0;
    for(int i = x86_bpred_gshare_hist; i >= 0; i--) {
       value[j] = x & (1u << i) ? '1' : '0';
	   j++;
    }
    fprintf(stderr, "%s\n", value);
...
int X86ThreadLookupBranchPred(X86Thread *self, struct x86_uop_t *uop)
{
	...
	/* Gshare predictor */
	if (x86_bpred_kind == x86_bpred_kind_gshare)
	{
...
void X86ThreadUpdateBranchPred(X86Thread *self, struct x86_uop_t *uop)
{
	...
	/* Gshare predictor was used */
	if (x86_bpred_kind == x86_bpred_kind_gshare)
	{
		pctr = &bpred->bimod[uop->gsindex ];
		*pctr = taken ? MIN(*pctr + 1, 3) : MAX(*pctr - 1, 0);
	}
...
struct x86_bpred_t *x86_bpred_create(char *name)
{
	...
	/* Bimodal predictor */
	if (x86_bpred_kind == x86_bpred_kind_bimod || x86_bpred_kind == x86_bpred_kind_comb || x86_bpred_kind == x86_bpred_kind_gshare)
	{
	...
	if(x86_bpred_kind == x86_bpred_kind_gshare){
		bpred->gshare_bhr = 0;
		bpred->gshare_displacement = (log2((double) x86_bpred_bimod_size) - x86_bpred_twolevel_hist_size);
	}		
...
char *x86_bpred_kind_map[] = { "Perfect", "Taken", "NotTaken", "Bimodal", "TwoLevel", "Combined", "Gshare"};
...
void X86ReadBranchPredConfig(struct config_t *config)
{
	...
	x86_bpred_kind = config_read_enum(config, section, "Kind", x86_bpred_kind_twolevel, x86_bpred_kind_map, 7);
	...
	x86_bpred_gshare_hist = config_read_int(config, section, "Gshare.HistorySize", 8);
	...
	if (x86_bpred_twolevel_l2size & (x86_bpred_twolevel_l2size - 1))
		fatal("two-level predictor sizes must be power of 2");
	if (x86_bpred_gshare_hist < 1 || x86_bpred_gshare_hist > 30 || x86_bpred_gshare_hist > log2((double) x86_bpred_bimod_size))
		fatal("predictor history size must be >=1 and <=30, and must be >= bimod size");
}
